    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if(SimpMessageType.SUBSCRIBE.equals(accessor.getCommand().getMessageType())) {
            ...
            if(tokenHeaders == null || tokenHeaders.isEmpty()) {
                throw new NotAuthorizedException("Missing token header");
            }
            if(userIdHeaders == null || userIdHeaders.isEmpty()) {
                throw new NotAuthorizedException("Missing userId header");
            }
            ...
            if(!hasAccessToProject) {
                throw new NotAuthorizedException("User " + userId + " does not have access to project " + projectId);
            }
